...
import { ChevronLeft, ChevronRight, Plus, Trash2, Pencil, X, Repeat, Target, Hash, Layers } from 'lucide-react'

export const EMPTY_GOAL = () => ({ text: '', type: 'habit', target: '1', unit: '', subGoals: [] })
export const EMPTY_SUB  = () => ({ text: '', target: '1', unit: '' })

// ── Single goal editor popup — shared by the member page, weekly goal
//    setup, and goal templates so adding/editing a goal is always the
//    same wizard (name → type → target/breakdown), regardless of where
//    it was opened from ────────────────────────────────────────────────
export function GoalPopup({ goal, onSave, onClose }) {
...
// ── Sortable goal card — same card language everywhere a goal list is
//    edited (member page, weekly goal setup, templates) ────────────────
export function SortableGoalRow({ id, text, type, target, unit, subGoals = [], onEdit, onDelete }) {
