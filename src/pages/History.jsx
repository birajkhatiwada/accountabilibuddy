...
export default function History() {
  ...
  if (weeks.length === 0) return (
    <div className="text-center py-20 space-y-3">
      ...
      <p className="font-semibold text-zinc-300">No history yet</p>
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-lg font-bold text-white">Past Weeks</h2>
      {weeks.map(weekId => {
        ...
        return (
          <div key={weekId} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
            <button
              onClick={() => setExpanded(p => ({ ...p, [weekId]: !p[weekId] }))}
              className="w-full flex items-center gap-3 p-4 text-left"
            >
              <div className="flex-1">
                <p className="font-semibold text-zinc-200 text-sm">{formatWeekLabel(weekId)}</p>
                ...
                {weekEntries.sort((a, b) => a.name.localeCompare(b.name)).map(e => (
                  <div key={e.id} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {e.updates?.length > 0 && (
                        <p className="text-xs text-zinc-600 mt-0.5">{e.updates.length} proof update{e.updates.length !== 1 ? 's' : ''}</p>
                      )}
                    </div>
                    {e.status === 'failed' && (
